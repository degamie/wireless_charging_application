...
public class FullScreenMainActivity extends AppCompatActivity {
    ...
    protected void OnCreate(Bundle savedInstanceState){
        super.onCreate(R.layout.activity_full_screen_main);
    ...
    private int getChargingSource(Intent intent){
        String SetText=null;//injtiailsize the Settext
        int Source=intent.getIntExtra("plugged",-1);//Source Declare
        switch(Source){//Switcha nd case Sys
            case BatteryManager.BATTERY_PLUGGED_WIRELESS://Wireless Sys
                chargingSource("WIRELESS");
                break;
            default://Battery Not Chargwed
                chargingSource("NULL");
        }
        return Source;//Printing Source
    }

    private String dictionaryEntries() {
        return "";
    }
    public void chargeBroadCast(Intent intent)//Chargign the soruce
    {
        IntentFilter intentFilter=new IntentFilter();//IntnetFILTter Obkjecte Declare
        intentFilter.addAction(Intent.ACTION_POWER_CONNECTED).show();//Power Connected Display
        registerReceiver(chargeBroadCast(intentFilter));//Registering athe power Reciever and with Allocated Power
...
//    }
//
//    public String BatteryManagement() {return "";}
